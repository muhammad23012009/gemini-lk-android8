...
int sec_get_pubk(unsigned char *pubk, unsigned int pubk_size);
...
{
	unsigned char public_key[PUBLIC_KEY_LENGTH] = {0};
	...
	if(sec_get_pubk(public_key, PUBLIC_KEY_LENGTH))
		dprintf(CRITICAL,"fail to get public key info\n");

	if(sha256(public_key, PUBLIC_KEY_LENGTH, public_key_hash))
	...
	boot_get_os_version(&os_version);
	if(os_version == 0)
		dprintf(CRITICAL,"fail to get os version and patch level\n");
#if 0
	for(int i=0; i<PUBLIC_KEY_LENGTH; i+=8)
