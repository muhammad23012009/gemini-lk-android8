...
#ifdef MTK_AB_OTA_UPDATER
	snprintf(partition_name, sizeof(partition_name), "%s%s", part_name, ab_suffix);
	part = mt_part_get_partition(partition_name);
	ALWAYS_LOG("mt_part_get_partition (%s)\n", partition_name);
#else
...
#else
	storage_start_addr = part->startblk * BLK_SIZE;
#endif
