...
int ccci_ld_md_sec_ptr_hdr_verify(char *partition_name, char *image_name)
{
...
#ifdef MTK_SECURITY_SW_SUPPORT
	policy_entry_idx = get_policy_entry_idx((unsigned char *)partition_name);
	...
	if (img_auth_required) {
...
#endif
		ret = sec_img_auth_init((unsigned char *)partition_name, (unsigned char *)image_name, sec_feature_mask);
		...
		if (!strncmp((char const *)image_name, "md1rom", 7)) {
			if (sec_set_md_default_pubk_hash) {
