...
static int find_hdr_in_partiton(char *partition_name, char *hdr_name, IMG_HDR_T *p_hdr, unsigned int p_hdr_size)
{
	int load_size;
	int hdr_offset = 0;
	...
	while(1) {
		...
		hdr_offset += p_hdr->info.hdr_size;
		hdr_offset += ((p_hdr->info.dsize + p_hdr->info.align_size - 1) & (~(p_hdr->info.align_size - 1)));
	}
}
				...
				unsigned char *mem_addr, unsigned int max_size, unsigned int flags)
{
	...
	while (partition_list[i] != NULL) {
		hdr_offset = find_hdr_in_partiton(partition_list[i], name, p_hdr, sizeof(IMG_HDR_T));
		if (hdr_offset < 0) {
			i++;
			continue;
		}
	...
	/* load image raw data */
	load_size = (int)ccci_load_raw_data(partition_list[i], mem_addr, hdr_offset + p_hdr->info.hdr_size,
						p_hdr->info.dsize);
	if ((unsigned int)load_size != p_hdr->info.dsize) {
		ALWAYS_LOG("load sub-image data fail(%d:%d)\n", load_size, p_hdr->info.dsize);
		ret = -LD_ERR_PT_LD_IMG_DATA_FAIL;
		goto _Exit;
	}

	/* Calcualte size that add padding */
	img_size_with_padding = (load_size + p_hdr->info.align_size - 1) & (~(p_hdr->info.align_size -1));
	/* Clear padding data to 0 */
	for (i = 0; i < img_size_with_padding - (int)p_hdr->info.dsize; i++)
		mem_addr[p_hdr->info.dsize + i] = 0;
...
static download_info_t md1_download_list_v20000[] = {/* for 92, 93 */
	...
	{dsp_img,    "md1dsp",   "md1dsp",    0x1800000,    0,         0,                NULL,      ass_func_for_dsp_normal_img},
	{armv7_img,  "md1arm7",  "md1arm7",   0x200000,    0,         0,                NULL,      NULL},
	{0,          NULL,       NULL,        0,           0,         0,                NULL,      NULL},
};
