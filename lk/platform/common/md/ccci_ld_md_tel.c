...
/* define it for one load support multi MD RAT mode */
#define MD_RAT_MODE_ALL_IN_ONE
#endif
/***************************************************************************************************
** Sub section:
...
#ifdef MD_RAT_MODE_ALL_IN_ONE
/* define MD different mode number */
#define MD_RAT_MODE_OFFSET 6
#define MD_3M_C2K 0
#define MD_3M_FDD (0 + MD_RAT_MODE_OFFSET)
#define MD_3M_TDD (0 + MD_RAT_MODE_OFFSET * 2)
#define MD_4M_C2K (0 + MD_RAT_MODE_OFFSET * 3)
#define MD_4M_FDD (0 + MD_RAT_MODE_OFFSET * 4)
#define MD_5M_C2K (0 + MD_RAT_MODE_OFFSET * 5)
#define MD_5M_FDD (0 + MD_RAT_MODE_OFFSET * 6)
#define MD_6M     (0 + MD_RAT_MODE_OFFSET * 7)

/* same with default_option_list */
static struct opt_cfg md_Xm_option_list[] = {
	/* C2k 3M (C/W/G) */
	{"opt_md1_support", 11},
	{"opt_md3_support", 0},
	{"opt_c2k_lte_mode", 2},
...
#endif /* end of MD_RAT_MODE_ALL_IN_ONE */
#else
...
#ifdef MD_RAT_MODE_ALL_IN_ONE
#define BYPASS_RAT_ALL -2
/* Customer: please fill this function
*return -1: Customer cannot support this function
*return 0 ~ 7: means Customer support this function
*/
#if 0
int get_md_type_from_custom(void)
{
	return -1;
}
#endif
#ifndef DUMMY_AP_MODE
int get_md_rat_mode_by_type(void)
{
	int type;

	/* if Customer doesn't support DT get SW load info, CCCI will bypass this function */
	type = -1;

	/* type = get_md_type_from_custom(); */

	/* used DT API instead of implementing this special API */
	extern unsigned customized_get_RAT_id(void) __attribute__((weak));

	if (customized_get_RAT_id)
		type = customized_get_RAT_id();
	else
		ALWAYS_LOG("don't support RAT all in one\n");

	if (type == -1)
		return BYPASS_RAT_ALL;
	...
	ALWAYS_LOG("cannot get MD rate mode!!! wrong type = %d\n", type);
	return -1;
}
#endif
#endif
...
int prepare_tel_fo_setting(void)
{
	...
	char *ptr;
#ifdef MD_RAT_MODE_ALL_IN_ONE
	/* this is for one image support multi MD rat mode,
	* this setting's prioirty is higher than all
	*/
	int type;
#endif
...
#ifndef DUMMY_AP_MODE
#ifdef MD_RAT_MODE_ALL_IN_ONE
			type = get_md_rat_mode_by_type();
			if (type == -1) {
				ALWAYS_LOG("get non-vaild MD rat mode type!!!\n");
				goto __RAT_ALL;
			} else if (type == BYPASS_RAT_ALL) {
				ALWAYS_LOG("bypass MD RAT mode all in one!!\n");
				goto __RAT_ALL;
			}
			DBG_LOG("get_md_rat_mode_by_type name = %s, val= %d\n",
			        md_Xm_option_list[type + i].name, md_Xm_option_list[type + i].val);
			value = md_Xm_option_list[type + i].val;
			/* the MD RAT mode all in one is the highest priority,
			*when this function opened, default and
			*lk env setting will be re-write by it
			*/
			default_option_list[i].val = md_Xm_option_list[type + i].val;
			ptr = NULL;
#endif
#endif
__RAT_ALL:
	...
	for (i = 0; i < cfg_item_num; i++) {
		...
		if (strcmp(default_option_list[i].name, "opt_md3_support") == 0) {
			value = 2;
			insert_ccci_tag_inf(default_option_list[i].name, (char*)&value, sizeof(int));
			ALWAYS_LOG("Always enable MD3!!!!!!!\n");
			default_option_list[i].val = 2;
		} else
...
#ifdef DUMMY_AP_MODE
			if (strcmp(opt, "opt_md3_support") == 0)
				return 2;
#endif
