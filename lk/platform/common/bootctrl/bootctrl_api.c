...
int check_suffix_with_slot(char *suffix)
{
    ...
    if(suffix == NULL) {
        dprintf(CRITICAL, "[LK] input suffix is NULL\n");
        return -1;
    }
    ...
    if(!strcmp(suffix, BOOTCTRL_SUFFIX_A)) {
        ...
        dprintf(CRITICAL, "[LK] unknow slot suffix\n");
    }
...
static int read_write_partition_info(boot_ctrl_t *bctrl ,int mode)
{
    ...
    if(bctrl == NULL) {
        dprintf(CRITICAL, "[LK] read_write_partition_info failed, bctrl is NULL\n");
        return ret;
    }
    ...
    if(mode == READ_PARTITION) {
        ...
        if (ret < 0) {
            dprintf(CRITICAL, "[LK] partition_read failed, ret: 0x%x\n", ret);
            return ret;
        }
    }
        ...
        if (ret < 0) {
            dprintf(CRITICAL, "[LK] read_write_partition_info partition_write failed, unknown mode\n");
            return ret;
        }
    }
    else {
        dprintf(CRITICAL, "[LK] unknown mode, ret: 0x%x\n", ret);
        return ret;
    }
...
const char *get_suffix(void)
{
    ...
    if (ret < 0) {
        dprintf(CRITICAL, "[LK] read_partition_info failed, ret: 0x%x\n", ret);
        return NULL;
    }

    if(metadata.magic != BOOTCTRL_MAGIC) {
        dprintf(CRITICAL, "[LK] boot_ctrl magic number is not match\n");
        return NULL;
    }
    else {
        dprintf(CRITICAL, "[LK] boot_ctrl magic number is match, compare priority\n");
...
int set_active_slot(char *suffix)
{
    ...
    if(slot == -1) {
        dprintf(CRITICAL, "[LK] set_active_slot failed, slot: 0x%x\n", slot);
        return -1;
    }
    ...
    if (ret < 0) {
        dprintf(CRITICAL, "[LK] partition_write failed, ret: 0x%x\n", ret);
        return -1;
    }
...
uint8_t get_retry_count(char *suffix)
{
    ...
    slot = check_suffix_with_slot(suffix);
    if(slot == -1) {
        dprintf(CRITICAL, "[LK] get_retry_count failed, slot: 0x%x\n", slot);
        return 0;
    }
...
int set_normal_boot(char *suffix, int boot_mode)
{
    ...
    slot = check_suffix_with_slot(suffix);
    if(slot == -1) {
        dprintf(CRITICAL, "[LK] set_not_normal_boot failed, slot: 0x%x\n", slot);
        return -1;
    }
...
/* Return value:
   ...
   -1: read misc partition error
*/
int get_bootable_status(char *suffix)
{
    int slot = 0, ret = -1;
    slot_metadata_t *slotp;
    boot_ctrl_t metadata;
