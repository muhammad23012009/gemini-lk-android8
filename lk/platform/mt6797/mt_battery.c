...
void check_bat_protect_status()
{
	...
	while (bat_val < BATTERY_LOWVOL_THRESOLD) {
		...
		if (pmic_get_register_value(MT6351_PMIC_RGS_BATON_UNDET) == 1) {
...
void do_ptim(void)
{
	...
	//pmic_auxadc_lock();
	//pmic_set_register_value(MT6351_PMIC_RG_AUXADC_RST,1);
	//pmic_set_register_value(MT6351_PMIC_RG_AUXADC_RST,0);
