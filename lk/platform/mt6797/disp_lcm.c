...
disp_path_handle _display_interface_path_init(disp_lcm_handle *plcm)
{
	...
	DISPCHECK("dpmgr set dst module(%s) done\n", ddp_get_module_name(dst_module));
...
void _dump_lcm_info(disp_lcm_handle *plcm)
{
	...
	DISPCHECK("*** dump lcm driver info ***\n");

	if (l && p) {
		DISPCHECK("name:%s\n", l->name);

		DISPCHECK("resolution:%dx%d\n", p->width, p->height);
		DISPCHECK("physical size:%dx%d\n", p->physical_width, p->physical_height);
		//DISPCHECK("physical size:%dx%d\n", p->physical_width, p->physical_height);
		DISPCHECK("lcm_if:%d,cmd_if:%d\n", p->lcm_if, p->lcm_cmd_if);
		switch (p->lcm_if) {
			case LCM_INTERFACE_DSI0:
				DISPCHECK("interface: DSI0\n");
				break;
			case LCM_INTERFACE_DSI1:
				DISPCHECK("interface: DSI1\n");
				break;
			case LCM_INTERFACE_DPI0:
				//DISPCHECK("interface: DPI0\n");
				break;
			case LCM_INTERFACE_DPI1:
				//DISPCHECK("interface: DPI1\n");
				break;
			case LCM_INTERFACE_DBI0:
				//DISPCHECK("interface: DBI0\n");
				break;
			default:
				DISPCHECK("interface: unknown\n");
				break;
		}
		...
		if (p->type == LCM_TYPE_DSI) {
			switch (p->dsi.mode) {
				case CMD_MODE:
					DISPCHECK("DSI Mode: CMD_MODE\n");
					break;
				case SYNC_PULSE_VDO_MODE:
					DISPCHECK("DSI Mode: SYNC_PULSE_VDO_MODE\n");
					break;
				case SYNC_EVENT_VDO_MODE:
					DISPCHECK("DSI Mode: SYNC_EVENT_VDO_MODE\n");
					break;
				case BURST_VDO_MODE:
					DISPCHECK("DSI Mode: BURST_VDO_MODE\n");
					break;
				default:
					DISPCHECK("DSI Mode: Unknown\n");
					break;
			}
		}

		if (p->type == LCM_TYPE_DSI) {
			DISPCHECK("LANE_NUM:%d,color_format:%d\n",
				  p->dsi.LANE_NUM, p->dsi.data_format.format);
#ifdef MY_TODO
#error
#endif
			DISPCHECK("vact:%d,vbp:%d,vfp:%d,vact_line:%d,hact:%d,hbp:%d,hfp:%d,hblank:%d,hactive:%d\n",
				  ...
				  p->dsi.horizontal_frontporch, p->dsi.horizontal_blanking_pixel, p->dsi.horizontal_active_pixel);
			DISPCHECK("pll_select:%d,pll_div1:%d,pll_div2:%d,fbk_div:%d,fbk_sel:%d,rg_bir:%d\n",p->dsi.pll_select,p->dsi.pll_div1,p->dsi.pll_div2,p->dsi.fbk_div,p->dsi.fbk_sel,p->dsi.rg_bir);
			DISPCHECK("rg_bic:%d,rg_bp:%d,PLL_CLOCK:%d,dsi_clock:%d,ssc_range:%d,ssc_disable:%d,compatibility_for_nvk:%d,cont_clock:%d\n", p->dsi.rg_bic, p->dsi.rg_bp,p->dsi.PLL_CLOCK,p->dsi.dsi_clock,p->dsi.ssc_range,p->dsi.ssc_disable,p->dsi.compatibility_for_nvk,p->dsi.cont_clock);
			DISPCHECK("lcm_ext_te_enable:%d,noncont_clock:%d,noncont_clock_period:%d\n", p->dsi.lcm_ext_te_enable,p->dsi.noncont_clock,p->dsi.noncont_clock_period);
		}
	}
