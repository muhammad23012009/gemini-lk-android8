...
#define MIPITX_OUTREG32(addr, val) \
    {\
        if(mipi_reg_op_debug) \
        {   DISPMSG("mipi/out:0x%08x=0x%08x\n", addr, val);}\
...
void DSI_PHY_CLK_LP_PerLine_config(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	DSI_PSCTRL_REG ps = { 0 };
	UINT32 hstx_ckl_wc, new_hstx_ckl_wc = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (0!=data_Rate) {
			...
			*/
			...
			DISPERR("[dsi]PLL clock should not be 0!\n");
			ASSERT(0);
		}
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	if (0 != dsi_params->PLL_CLOCK) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "DSI_PHY_TIMCONFIG,Cycle Time=%dns,Unit Interval=%dns,lane#=%d\n", cycle_time, ui, lane_no);
	} else {
		DISPERR("[dsi]PLL clock should not be 0!\n");
		ASSERT(0);
	}
...
#define NS_TO_CYCLE(n, c)   ((n) / (c))
	...
	DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", "DSI_PHY_TIMCONFIG,HS_TRAIL=%d,HS_ZERO=%d,HS_PRPR=%d,LPX=%d,TA_GET=%d,TA_SURE=%d,TA_GO=%d,CLK_TRAIL=%d,CLK_ZERO=%d,CLK_HS_PRPR=%d\n",\
...
void DSI_set_cmdq(DISP_MODULE_ENUM module, void* cmdq, unsigned int *pdata, unsigned int queue_size, unsigned char force_update)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (0 != DSI_REG[i]->DSI_MODE_CTRL.MODE) {
			...
			for (j = 0; j < queue_size; j++)
				dprintf(INFO,"[DISP]DSI_set_cmdq. DSI_CMDQ+%04x: 0x%08x\n", j*4, INREG32(DSI0_BASE + 0x200 + j*4));
...
void _dump_dsi_params(LCM_DSI_PARAMS *dsi_config)
{
	...
	if (dsi_config) {
		...
		DISPCHECK("[DSI]LANE_NUM:%d,color_format:%d\n", dsi_config->LANE_NUM, dsi_config->data_format.format);

		DISPCHECK("[DSI]vsa:%d,vbp:%d,vfp:%d,vact_line:%d,hact:%d,hbp:%d,hfp:%d,hblank:%d\n",
