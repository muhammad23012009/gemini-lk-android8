           ...
           void * handle)
{
	...
	if ((bgW > OVL_MAX_WIDTH) || (bgH > OVL_MAX_HEIGHT)) {
		DDPERR("OVLROI(): exceed OVL max size(%dx%d)\n", bgW, bgH);
		ASSERT(0);
	}
                   ...
                   void * handle)
{
	...
	DDPDBG("ovl%d,layer=%d,source=%s,off(%d,%d),dst(%d,%d,%dx%d),pitch=%d,"
	       "fmt=%s,addr=%lx,keyEn=%d,key=%d,aen=%d,alpha=%d\n",
	       idx, layer, (source==0)?"memory":"constant_color",
	       src_x, src_y, dst_x, dst_y, dst_w, dst_h, src_pitch,
	       ovl_intput_format_name(fmt, input_swap), addr,
	       keyEn, key, aen, alpha);
...
void OVLRegStore(DISP_MODULE_ENUM module)
{
	...
	DDPDBG("store %d cnt regs on ovl %d", reg_back_cnt[idx], idx);
...
void OVLRegRestore(DISP_MODULE_ENUM module,void * handle)
{
	...
	DDPDBG("restore %d cnt regs on ovl %d", reg_back_cnt[idx], idx);
	reg_back_cnt[idx] = 0;
}
...
{
	int idx = ovl_index(module);
	unsigned int src_con = DISP_REG_GET(DISP_REG_OVL_SRC_CON + DISP_INDEX_OFFSET * idx);
	int i = 0;
	...
	DDPMSG("ROI_SIZE=0x%x\n", DISP_REG_GET(DISP_REG_OVL_ROI_SIZE+DISP_INDEX_OFFSET*idx));
	DDPMSG("PATH_CON=0x%x\n", DISP_REG_GET(DISP_REG_OVL_DATAPATH_CON+DISP_INDEX_OFFSET*idx));
	DDPMSG("ROI_BGCLR=0x%x\n", DISP_REG_GET(DISP_REG_OVL_ROI_BGCLR+DISP_INDEX_OFFSET*idx));
	DDPMSG("SRC_CON=0x%x\n", DISP_REG_GET(DISP_REG_OVL_SRC_CON+DISP_INDEX_OFFSET*idx));

	for (i = 0; i < ovl_layer_num(module); i++) {
		if (src_con & (0x1 << i)) {
			DDPMSG("L%d:\n", i);
			DDPMSG("CON=0x%x\n", DISP_REG_GET((DISP_REG_OVL_L0_CON + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("SRCKEY=0x%x\n", DISP_REG_GET((DISP_REG_OVL_L0_SRCKEY + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("SRC_SIZE=0x%x\n", DISP_REG_GET((DISP_REG_OVL_L0_SRC_SIZE + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("OFFSET=0x%x\n", DISP_REG_GET((DISP_REG_OVL_L0_OFFSET + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("ADDR=0x%x\n", DISP_REG_GET((DISP_REG_OVL_L0_ADDR + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("PITCH=0x%x\n", DISP_REG_GET((DISP_REG_OVL_L0_PITCH + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("TILE=0x%x\n", DISP_REG_GET((DISP_REG_OVL_L0_TILE + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("RDMA_CTRL=0x%x\n", DISP_REG_GET((DISP_REG_OVL_RDMA0_CTRL + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("RDMA_MEM_GMC_SET1=0x%x\n", DISP_REG_GET((DISP_REG_OVL_RDMA0_MEM_GMC_SETTING + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("RDMA_MEM_SLOW_CON=0x%x\n", DISP_REG_GET((DISP_REG_OVL_RDMA0_MEM_SLOW_CON + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			DDPMSG("RDMA_FIFO_CTRL=0x%x\n", DISP_REG_GET((DISP_REG_OVL_RDMA0_FIFO_CTRL + 0x20 * i) + DISP_INDEX_OFFSET * idx));
			//DDPMSG("(0x1e0)O0_R_M_GMC_SET2 =0x%x\n", DISP_REG_GET(DISP_REG_OVL_RDMA0_MEM_GMC_SETTING2+DISP_INDEX_OFFSET*idx));
			DDPMSG("RDMA_DBG=0x%x\n", DISP_REG_GET((DISP_REG_OVL_RDMA0_DBG + 0x4 * i) + DISP_INDEX_OFFSET * idx));
		}
	}

	DDPMSG("DBG_MON_SEL=0x%x\n", DISP_REG_GET(DISP_REG_OVL_DEBUG_MON_SEL+DISP_INDEX_OFFSET*idx));
	DDPMSG("DUMMY_REG=0x%x\n", DISP_REG_GET(DISP_REG_OVL_DUMMY_REG+DISP_INDEX_OFFSET*idx));
	DDPMSG("FLOW_CTRL=0x%x\n", DISP_REG_GET(DISP_REG_OVL_FLOW_CTRL_DBG+DISP_INDEX_OFFSET*idx));
...
void ovl_dump_analysis(DISP_MODULE_ENUM module)
{
	...
	DDPMSG("ovl_en=%d,layer_enable(%d,%d,%d,%d),bg(%dx%d),"
...
static int OVLConfig_l(DISP_MODULE_ENUM module, disp_ddp_path_config* pConfig, void* handle)
{
	...
	for (local_layer = 0; local_layer < ovl_layer_num(module); local_layer++, global_layer++) {
		...
		if (ovl_cfg->addr==0 || ovl_cfg->dst_w==0 || ovl_cfg->dst_h==0) {
			DDPERR("invalid ovl params: addr=0x%x,w=%d,h=%d\n",
			       ovl_cfg->addr,
			       ovl_cfg->dst_w,
			       ovl_cfg->dst_h);
			return -1;
		...
		DDPDBG("%s,layer=%d,en=%d,src=%d,fmt=%d,addr=0x%x,x=%d,y=%d,pitch=%d,dst(%d,%d,%dx%d),keyEn=%d,key=%d,aen=%d,alpha=%d\n",
		       ddp_get_module_name(module),
		       local_layer,
		       ovl_cfg->layer_en,
