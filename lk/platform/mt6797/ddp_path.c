...
static int  ddp_find_module_index(DDP_SCENARIO_ENUM ddp_scenario, DISP_MODULE_ENUM module)
{
	...
	DDPDBG("find module: can not find module %s on scn %s\n",ddp_get_module_name(module),
...
int ddp_set_dst_module(DDP_SCENARIO_ENUM scenario, DISP_MODULE_ENUM dst_module)
{
	int i = 0;
	int j = 0;
	DDPMSG("ddp_set_dst_module, scn=%s, dst_module=%s\n",
...
{
	DDPMSG("ddp_get_dst_module, scn=%s, dst_module=%s\n",
	       ddp_get_scenario_name(ddp_scenario),
...
{
	DDPDBG("path disconnect on scn %s\n", ddp_get_scenario_name(scenario));
...
{
	DDPDBG("path check path on scn %s\n", ddp_get_scenario_name(scenario));
