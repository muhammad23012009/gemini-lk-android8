...
int load_scp_image(char *part_name, char *img_name, void *addr)
{
	...
	if (img_auth_required) {
		...
		sec_ret = sec_img_auth_init(part_name, img_name, sec_feature_mask);
		if (sec_ret)
			return -1;
		...
		if (sec_ret) {
			dprintf(DEBUG, "[SBC] ver check fail (%x)\n", sec_ret);
			return -1;
		}
#endif
	}
...
#define SCP_PARTITION_NAME_SIZE    10
    ...
    dram_addr = (void *) (unsigned int) mblock_reserve_ext(&g_boot_arg->mblock_info, SCP_DRAM_SIZE, 0x10000, DRAM_ADDR_MAX, 0, "SCP-reserved");
