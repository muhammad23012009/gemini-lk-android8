...
static u64 get_gpt_header_last_usable_lba()
{
	...
	return (last_lba(EMMC_PART_USER)-((blksz+PART_MAX_COUNT*sizeof(gpt_entry))/blksz));
}
...
static int parse_gpt_header(u32 part_id, u64 header_lba, u8 *header_buf, u8 *entries_buf)
{
	...
	err = read_data(header_buf, part_id, header_lba, blksz);
	if (err) {
	...
	entries_read_size = (u64)((header->num_partition_entries + 3) / 4) * blksz;
...
int read_gpt(part_t *part)
{
	...
	pgpt_header = (u8 *)malloc(blksz);
	if (!pgpt_header) {
		efi_err("%smalloc memory(pgpt header), err\n", TAG);
		goto next_try;
	}
	memset(pgpt_header, 0, blksz);
...
next_try:
	...
	sgpt_header = (u8 *)malloc(blksz);
	if (!sgpt_header) {
		efi_err("%smalloc memory(sgpt header), err\n", TAG);
		return 1;
	}
	memset(sgpt_header, 0, blksz);
...
int write_primary_gpt(void *data, unsigned sz)
{
	...
	free(pmbr); /* Free pmbr after use it */
	if (err) {
