...
void log_store_init(void)
{
	...
	LOG_DEBUG("%s:sram buff header 0x%x,buff address 0x%x, sig 0x%x, buff_size 0x%x, pl log size 0x%x@0x%x, lk log size 0x%x@0x%x!\n",
	          MOD, (unsigned int)sram_header, (unsigned int)buff_header, (unsigned int)buff_header->sig, (unsigned int)buff_header->buff_size,
	          (unsigned int)buff_header->sz_pl, (unsigned int)buff_header->off_pl, (unsigned int)buff_header->sz_lk, (unsigned int)buff_header->off_lk);
...
{
	return sram_dram_buff->mirror_header_addr;
}

u32 mirror_buf_pl_lk_log_size_get(void)
{
	return LOG_STORE_SIZE_MIRROR;
}
