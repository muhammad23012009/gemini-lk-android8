...
part_t *mt_part_get_partition(char *name)
{
	...
	u32 bootpart=1; //default bootpart = 1
...
void mt_part_dump(void)
{
	...
	part_info("Part Info.(1blk=%luB):\n", blksz);
	while (part->nr_sects) {
		part_info("[0x%016llx-0x%016llx] (%.8ld blocks): \"%s\"\n",
		          (u64)part->start_sect * blksz,
		          (u64)(part->start_sect + part->nr_sects) * blksz - 1,
...
void mt_part_dump(void)
{
	...
	while (part->name) {
		part_info("[0x%.8x-0x%.8x] (%.8ld blocks): \"%s\"\n",
		          part->start_sect * blksz,
		          (part->start_sect + part->nr_sects) * blksz - 1,
