...
#ifdef MTK_EMMC_SUPPORT
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
	len = dev->read(dev, start_addr + sizeof(part_hdr_t), (uchar *)addr,
			part_hdr->info.dsize, part->part_id);
#else
	len = dev->read(dev, start_addr + sizeof(part_hdr_t), (uchar *)addr,
			part_hdr->info.dsize);
#endif
...
#ifdef MTK_EMMC_SUPPORT
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
	len = dev->read(dev, addr, (uchar *) boot_hdr, sizeof(struct bootimg_hdr),
			part->part_id);
#else
	len = dev->read(dev, addr, (uchar *) boot_hdr, sizeof(struct bootimg_hdr));
#endif
...
int mboot_android_load_bootimg(char *part_name, unsigned long addr)
{
	...
	//*
#ifdef MTK_EMMC_SUPPORT
	start_addr = (u64)part->start_sect * BLK_SIZE + g_boot_hdr->page_sz;
#else
	start_addr = part->start_sect * BLK_SIZE + g_boot_hdr->page_sz;
#endif
...
unsigned int mboot_get_inhouse_img_size(char *part_name, unsigned int *size)
{
	...
	*size = 0;
	...
	/*Read mkimage header*/
#if defined(MTK_EMMC_SUPPORT) && defined(MTK_NEW_COMBO_EMMC_SUPPORT)
	len = dev->read(dev, addr, (uchar *)&mkimage_hdr, sizeof(part_hdr_t),
			part->part_id);
#else
	len = dev->read(dev, addr, (uchar *)&mkimage_hdr, sizeof(part_hdr_t),
			part->part_id);
