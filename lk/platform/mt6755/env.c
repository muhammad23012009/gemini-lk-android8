...
static int write_env_area(char *env_buf, unsigned int area)
{
	...
	*((int *)env_buf+CFG_ENV_CHECKSUM_OFFSET/4) = checksum;
	...
	len = dev->write(dev, (uchar*)env_buf, start_addr + CFG_ENV_RW_OFFSET, CFG_ENV_SIZE, part->part_id);
#else
	len = -1;
#endif
	if (len < 0) {
...
static int read_env_area(char *env_buf, unsigned int area)
{
	...
	long len;
#ifdef MTK_EMMC_SUPPORT
	unsigned long long start_addr, offset;
#elif defined(MTK_NAND_SUPPORT)
	unsigned long start_addr;
#endif
...
#if defined(MTK_EMMC_SUPPORT) && defined(MTK_NEW_COMBO_EMMC_SUPPORT)
	len = dev->read(dev, start_addr + offset, (uchar*)env_buf, CFG_ENV_SIZE, part->part_id);
#else
	len = -1;
#endif
	if (len < 0)
