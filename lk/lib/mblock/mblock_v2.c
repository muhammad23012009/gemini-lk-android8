		  ...
		  u64 addr, u64 oldsize, u64 newsize)
{
	...
	for (i = 0; i < mblock_info->mblock_num; i++) {
		...
		sz = mblock_info->mblock[i].size;
		/* invalid mblock */
		if (((start >= addr) && (start < (addr + oldsize))) || ((start < addr) && (start + sz > addr))) {
			dprintf(CRITICAL, "mblock_resize error: addr %llx, oldsize: %llx  start: 0x%llx sz: 0x%llx is free\n",
	...
	found = 0;
	for (i = 0; i < mblock_info->reserved_num; i++) {
		dprintf(CRITICAL, "mblock_resize  start: %llx, size: %llx addr = %llx oldsize = %llx\n",
...
static int reserved_memory_conflict_check(void *fdt, mblock_info_t *mblock_info)
{
	...
	for (reserved_node = fdt_first_subnode(fdt, reserved_memory_offset); reserved_node >= 0;
		...
		dprintf(INFO, "DTS reserved start: 0x%llx size: 0x%llx\n", start, sz);
		for (i = 0; i < mblock_info->reserved_num; i++) {
