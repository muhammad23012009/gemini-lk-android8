...
#if defined(MTK_UFS_BOOTING) || defined(MTK_NEW_COMBO_EMMC_SUPPORT) || defined(MTK_TLC_NAND_SUPPORT)
...
static struct kzip_memlist *mrdump_memlist_fill(void)
{
	struct kzip_memlist *memlist = malloc(sizeof(struct kzip_memlist) * 4);
	if (memlist == NULL) {
		return NULL;
	}
	...
	memlist[1].need2map = false;
	memlist[2].address = DRAM_PHY_ADDR;
	memlist[2].size = mrdump_mem_size();
	memlist[2].need2map = true;
	memlist[3].address = 0;
	memlist[3].size = 0;
	memlist[3].need2map = false;
	return memlist;
}

static void mrdump_memlist_free(struct kzip_memlist *memlist)
{
...
static void kdump_ui(struct mrdump_control_block *mrdump_cblock)
{
	...
	struct kzip_memlist *memlist = mrdump_memlist_fill();
	if (memlist == NULL) {
...
error:
	...
	mrdump_usb_dump_select(mrdump_cblock);
	return 1;
}
