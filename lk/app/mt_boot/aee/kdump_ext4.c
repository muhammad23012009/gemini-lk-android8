...
static void dump_infolba(unsigned int *iLBA, const char *msg, ...)
{
	va_list ap;
	va_start(ap, msg);
	voprintf('E', msg, ap);
	...
	voprintf_info(" 1ST_LBA=%u, 2ND_LBA=%u, CORE_DUMP_SIZE=%u, USER_FILESIZE=%u, INFOBLOCK_CRC=0x%x!\n",
			iLBA[EXT4_1ST_LBA], iLBA[EXT4_2ND_LBA],
			iLBA[EXT4_CORE_DUMP_SIZE], iLBA[EXT4_USER_FILESIZE],
			iLBA[EXT4_INFOBLOCK_CRC]);
}

int mrdump_ext4_output(const struct mrdump_control_block *mrdump_cb, const struct kzip_memlist *memlist, struct mrdump_dev *mrdump_dev)
{
	...
	if (mycrc != InfoLBA[EXT4_INFOBLOCK_CRC]) {
		dump_infolba(InfoLBA, " InfoLBA CRC32 Error! Abort! (CRC1=0x%08x, CRC2=0x%08x)\n", mycrc, InfoLBA[EXT4_INFOBLOCK_CRC]);
		return -1;
	}
	...
	if (!handle->dumpdev->read(handle->dumpdev, ext4_lba_to_block_offset(handle->rlba), (uint8_t *)handle->block_lba, BLKSIZE)) {
		dump_infolba(InfoLBA, " SDCard: Reading BlockLBA error.\n");
		free(handle);
		return -1;
	}
