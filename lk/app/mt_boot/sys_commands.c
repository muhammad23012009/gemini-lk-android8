...
{
	voprintf_debug("MRDUMP USB DUMP rebooting...\n");
...
{
	voprintf_debug("MRDUMP USB DUMP Go!\n");
...
#define CMDLINE_TMP_CONCAT_SIZE 100     //only for string concat, 100 bytes is enough
	...
	if (mboot_android_check_img_info(PART_KERNEL,
					 (part_hdr_t *)(ptr + boot_hdr->page_sz)) == -1) {
		...
		if (g_is_64bit_kernel)
			memmove((void *) SCRATCH_ADDR, (ptr + boot_hdr->page_sz + MKIMG_HEADER_SZ),
				boot_hdr->kernel_sz);

		else
			memmove((void *) boot_hdr->kernel_addr,
				(ptr + boot_hdr->page_sz + MKIMG_HEADER_SZ), boot_hdr->kernel_sz);
	}
...
#endif
	if ((mboot_android_check_img_info(PART_ROOTFS,
					  (part_hdr_t *)(ptr + boot_hdr->page_sz + kernel_actual)) == -1)
	    && (mboot_android_check_img_info("RECOVERY",
					     (part_hdr_t *)(ptr + boot_hdr->page_sz + kernel_actual)) == -1)) {
		...
		memmove((void *) boot_hdr->ramdisk_addr,
			(ptr + boot_hdr->page_sz + kernel_actual + MKIMG_HEADER_SZ),
			boot_hdr->ramdisk_sz);
	}
