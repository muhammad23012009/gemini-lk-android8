...
#include <ufs_aio_hcd.h>
	#define HEADER_SIZE UFS_BLOCK_SIZE
#else
	#define HEADER_SIZE SDMMC_HEADER_SIZE
#endif
...
int process_preloader (char* data, /*IN OUT*/unsigned int* psz)
{
	...
	memcpy(mmcHdr->id, HEADER_IDENTIFIER, strlen(HEADER_IDENTIFIER));
	mmcHdr->version = SDMMC_HEADER_VERSION;
	mmcHdr->record_region_size = HEADER_SIZE;
...
unsigned int handle_layout_header(char* data, unsigned int sz, char* cache)
{
	...
	length += (preloader_max_size - HEADER_BLOCK_SIZE);
